    ...
    Column(
        ...
        Card(shape = RoundedCornerShape(14.dp)) {
            Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
                ...
                // v2.3.5: 旧「出口は52週高値-3%回復・8回全勝（検証17）」を最新の結論に差し替え
                Rule("買ったら売らない。出口は撤退線のみ",
                    "旧ルール「52週高値-3%回復で全売却・8回全勝（検証17）」は10年・比較相手なしの数字。" +
                    "12ヶ月で売って回すと0.91倍、売らずに持つと1.96倍で、差は市場にいた時間だけ" +
                    "（検証57。1306でも同じ向き＝検証50）。-3%回復は撤退ロックの解除条件としてだけ使う。" +
                    "トレーリングストップは勝率38%で厳禁（検証17）")
                Rule("点灯は待つ理由ではなく、上乗せで買う合図",
                    "資金を点灯まで寝かせるのは、同じ回数をランダムな日に入れるより悪い（検証69）。" +
                    "2013年以降は何もない日に買っても12M中央値+13.3%で、点灯条件" +
                    "（-15%または5日-8%）の超過は-2.7pt＝高い勝率は相場の手柄（検証167）")
                ...
                Rule("日経VI≥30 は「金額を厚くする」材料（点灯条件ではない）",
                    "VI≥30が重なった点灯は買値が底に近く（底より+18.6%高い→+9.9%）12M成績も" +
                    "+44.0%→+69.6%。ただし2016年以降のn=8と標本が小さいので出動の可否は変えず、" +
                    "満額か半分かの判断だけに使う（検証33）")
                Rule("含み損ではなく指数で切る",
